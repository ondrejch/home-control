...
GMC_PORT_CANDIDATES = tuple(
    p.strip() for p in os.environ.get("GMC_PORTS", "/dev/ttyUSB1").split(",") if p.strip()
)
...
def can_open_port(port: str) -> bool:
    """Return True if a serial port can be opened and responds to GETVER.

    Args:
        port: Device path such as `/dev/ttyUSB0`.
    """
    try:
        sr = serial.Serial(port, SERIAL_BAUDRATE, timeout=SERIAL_READ_TIMEOUT_SECONDS)
        sr.reset_input_buffer()
        sr.reset_output_buffer()
        sr.write("<GETVER>>".encode("ascii"))
        response = sr.readline().decode().strip()
        sr.close()
        if response:
            return True
        return False
...
def detect_port(candidates: Optional[Sequence[str]] = None) -> Optional[str]:
    """Detect the first accessible GMC serial port.

    Args:
        candidates: Ordered list of serial device paths to probe.
                    If None, dynamically scans `/dev/ttyUSB*` first, then
                    falls back to `GMC_PORT_CANDIDATES` from env var.

    Returns:
        First reachable port path, or None if no candidate is available.
    """
    if candidates is None:
        # Dynamically scan all USB serial ports after reboot.
        candidates = glob.glob("/dev/ttyUSB*")
    for port in candidates:
        if can_open_port(port):
            return port
    # Fallback to explicit env-var overrides if dynamic scan found nothing.
    if candidates != glob.glob("/dev/ttyUSB*"):
        pass
    else:
        for port in GMC_PORT_CANDIDATES:
            if can_open_port(port):
                return port
    return None
